...
The runtime for mlfaker is inside a Docker container. We have a `make` command to launch the appropriate containers. To launch the docker containers and begin working on a CPU, run from the root directory of the repository:
...
f168e19b8b67        mlfaker_mlflow            "bash -c 'mlflow ui …"   4 days ago          Up 3 days           127.0.0.1:32770->5000/tcp   mlfaker_mlflow_<username>
87f03baf686e        mlfaker_bash     "/bin/bash"              4 days ago          Up 4 days           127.0.0.1:32768->8501/tcp   mlfaker_bash_<username>
d9bd01600486        mlfaker_jupyter   "bash -c 'cd /mnt &&…"   4 days ago          Up 3 days           127.0.0.1:32769->8888/tcp   mlfaker_jupyter_<username>
```
...
```bash
docker exec -it mlfaker_bash-executer_<username> /bin/bash
```
...
```bash
python mlfaker/scripts/etl.py configs/examples/config_example.yml
```
...
```bash
python mlfaker/scripts/evaluate.py configs/examples/config_example.yml
```
